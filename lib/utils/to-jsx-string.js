...
import sobj from 'stringify-object'; // import prettier from 'prettier/standalone';
...
function renderWithoutChildren(element, level) {
  ...
  return ["".concat(indentation, "<").concat(type).concat(renderProps(props), " />")];
}
...
export default function toJsxString(element) {
  var code = renderNode(element, 0).map(function (l, i, a) {
